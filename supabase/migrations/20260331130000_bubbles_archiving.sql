-- ============================================================
-- Bubbles Tracking: archiving + daily snapshots (09:00 Asia/Baghdad)
-- الهدف: بعد 9 صباحاً يُؤرشف كل شيء ما عدا pending
--       وعند حل pending (completed/issue) يُؤرشف فوراً
-- ============================================================

-- 1) Archive table (تفاصيل كاملة)
...
-- SELECT: admin/manager/logistics فقط (يمكن توسيعها لاحقاً)
...
-- Admin-only mutations (عادةً لا نحتاجها لأن الأرشفة تتم عبر SECURITY DEFINER)
...
-- 2) Daily snapshots (تقرير يومي سريع)
...
-- 4) RPC: archive daily (moves all NON-pending out of bubbles_records)
...
BEGIN
  -- Allow service_role (scheduled job) OR admin
  ...
  -- Move non-pending rows into archive
  ...
  -- Build snapshot based on archived_day = v_day + current pending left in main
  SELECT
    count(*)::int,
...
-- 5) Trigger: on resolve pending -> archive immediately
...
DECLARE
  v_day DATE;
BEGIN
  -- Only when status changes from pending/delayed to completed/issue
