-- ============================================================
-- إصلاح سريع: السماح لمسؤول الصيانة بإنهاء الصيانة
-- نفّذ هذا في Supabase SQL Editor
-- ============================================================

-- تحديث التحقق من الدور ليشمل maintenance_manager
...
BEGIN
  ...
  -- Admin و maintenance_manager يمكنهم إنهاء الصيانة
  ...
  INSERT INTO public.maintenance_records (request_id, vehicle_id, maintenance_type, fault_description, work_done, inspection_only, parts_replaced, technician_name, cost, duration_minutes, notes)
  VALUES (p_request_id, v_request.vehicle_id, p_maintenance_type, p_fault_description, p_work_done, p_inspection_only, p_parts_replaced, p_technician_name, p_cost, p_duration_minutes, p_notes)
  ...
  UPDATE public.maintenance_requests SET status = 'completed', finished_at = now() WHERE id = p_request_id;
  INSERT INTO public.vehicle_events (vehicle_id, event_type, description, old_value, new_value)
  VALUES (v_request.vehicle_id, 'status_changed', 'صيانة مكتملة: ' || p_maintenance_type, 'maintenance', 'available');
  ...
  VALUES (v_request.vehicle_id, 'maintenance_completed', 'صيانة مكتملة', 'تم إنهاء صيانة المركبة: ' || p_maintenance_type || ' — التكلفة: ' || COALESCE(p_cost::TEXT, '0') || ' د.ع', 'admin');
