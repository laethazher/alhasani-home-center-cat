    ...
    if (data) {
      ...
      if (isGateGuard) {
        ...
        if (approvedCount > prevApprovedCount.current && prevApprovedCount.current > 0) {
          setFlashAt(new Date().toLocaleTimeString('ar-IQ', { hour: '2-digit', minute: '2-digit' }));
  ...
  const handleCreate = async () => {
    ...
    if (!error && insertedRequest) {
      const { error: gateNotifErr } = await supabase.from('gate_notifications').insert({
        ...
        target_role: 'gate_guard',
        metadata: {
          exit_request_id: insertedRequest.id,
          exit_type: formExitType,
        },
      });
      if (gateNotifErr) console.warn('تعذر إرسال إشعار موحد للبوابة:', gateNotifErr.message);
    }
