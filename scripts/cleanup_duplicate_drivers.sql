-- ================================================================
-- تنظيف أسماء السائقين المتكررة وحذف غير المرتبطين بمركبات
...
BEGIN
  ...
  LOOP
    ...
    IF v_dup_id IS NOT NULL AND v_keep_id IS NOT NULL AND v_dup_id <> v_keep_id THEN
      ...
      v_merged := v_merged + 1;
      RAISE NOTICE 'دُمج: حُذف id=% ← أُبقي id=%', v_final_del, v_final_keep;
    END IF;
  ...
  RAISE NOTICE '=== النتيجة: تم دمج % زوج مكرر + حذف % سائق غير مرتبط ===', v_merged, v_unlinked_del;
END $$;
